...
EOL
  ...
  # DO NOT modify existing README.md content here - only append to it later
fi
...
if ! git remote | grep -q origin; then
  ...
  if ! git diff --cached --quiet; then
    ...
    # Only add README.md if it exists
    if [ -f README.md ]; then
      git add README.md
      if ! git diff --cached --quiet; then
      ...
      echo "✔️  No README.md found. Repo fully synced."
    fi
  fi
fi
...
if [ -f README.md ]; then
  echo "📄 Updating README.md with project status (preserving existing content)"
  
  # Check if README already has a pushhard tag section
  if grep -q "<!-- PUSHHARD-TAG -->" README.md; then
    echo "🔄 Removing existing PUSHHARD-TAG section"
  ...
  # Add custom pushhard footer
  echo "📝 Adding PUSHHARD-TAG section to bottom of README.md"
  cat <<EOL >> README.md
...
Git Pushed Hard by Pushhard© Keep Pushin' with one word: Pushhard!
EOL
fi
